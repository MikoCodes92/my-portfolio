...
const projects: Project[] = [
  {
    title: "LaserVacBot",
    description: "Advanced robot vacuum simulation with Dijkstra's pathfinding algorithm for optimal cleaning routes",
  ...
  },
  {
    ...
    description: "Full-stack booking platform with real-time availability and WebSocket notifications",
  ...
  },
  {
    ...
    description: "Medical AI system for detecting pneumonia from chest X-ray images using convolutional neural networks",
  ...
  },
  {
    ...
    description: "Machine learning model for predictive analysis and optimization of urban traffic flow patterns",
  ...
  },
  {
    ...
    description: "Intelligent university course scheduling system using genetic algorithms for optimal resource allocation",
    tech: ["ASP.NET", "MSSQL", "Genetic Algorithms", "jQuery", "SSRS"],
    github: "https://github.com/MikoCodes92/Automatic-University-Scheduling-System",
    demo: "#",
  },
];
...
const ProjectCard3D: React.FC<{ project: Project; isMobile: boolean }> = memo(({ 
  ...
  useFrame(({ clock }) => {
    ...
    if (meshRef.current) {
      ...
      meshRef.current.rotation.x = Math.sin(t * 0.2) * 0.03;
  ...
  return (
    <Float 
      floatIntensity={hovered ? 0.3 : 0.15} 
      rotationIntensity={hovered ? 0.1 : 0.05}
      speed={2}
    >
      <mesh 
        ref={meshRef} 
        scale={isMobile ? [6, 8, 1] : [3, 4, 1]}
        onPointerEnter={() => setHovered(true)}
        onPointerLeave={() => setHovered(false)}
      >
        <boxGeometry args={isMobile ? [6, 8, 0.1] : [3, 4, 0.1]} />
        ...
        >
          ...
          <Card className="relative backdrop-blur-2xl bg-gradient-to-br from-gray-900/95 via-[#100c1f]/95 to-gray-900/95 border border-white/20 shadow-2xl w-full h-full p-6 sm:p-8 md:p-12 mx-auto text-center rounded-3xl overflow-hidden group">
            ...
            <div className={`absolute inset-0 rounded-3xl bg-gradient-to-br from-cyan-500/40 via-transparent to-cyan-400/40 opacity-0 group-hover:opacity-80 transition-all duration-700 blur-xl`} />
...
export const Projects: React.FC = () => {
  ...
  return (
    <section id="projects" className="py-16 px-4 w-full relative min-h-screen bg-transparent">
      ...
      >
        ...
        >
          ...
          <pointLight position={[-5, -5, -5]} intensity={0.3} color={COLORS.primary} />
          
          {/* Optimized Sparkles with cyan color */}
          <Sparkles
            count={isMobile ? 15 : 80}
            size={isMobile ? 0.4 : 0.8}
            scale={isMobile ? [12, 6, 12] : [20, 10, 20]}
            speed={0.3}
            color={COLORS.primary}
          />

          {/* Navigation Arrows */}
          {!isMobile && <NavigationArrows onPrev={handlePrev} onNext={handleNext} />}
